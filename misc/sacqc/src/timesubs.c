...
void setWeekOfYear( MyTime *t )
{
	MyTime a;
	int WeeksSinceYear0 = 0; /*** number of weeks since beginning of year ***/
	int jday;

	void initialize_mytime( MyTime *a );
	void complete( MyTime *a );
	void setDayOfWeek( MyTime *a );
	void WriteMyTime2STDOUT( MyTime *a );
	void setDayOfWeek( MyTime *a );

	for( jday = 1; jday <= t->jday; jday++ )
	{
		initialize_mytime( &a );
		a.year = t->year;
		a.jday = jday;
		...
		/* WriteMyTime2STDOUT( &a ); */
		/* fprintf( stdout, "%d %d %d\n", 
			jday, a.dayofweek, WeeksSinceYear0 );
		*/

		if( a.dayofweek == 0 )
			WeeksSinceYear0++;
	}
	t->weekofyear = WeeksSinceYear0; 
/*** first week is zero, so 52wks in year ****/
}
...
/***************************************************************************
       clone time t to time a
****************************************************************************/
void mytime_clone( MyTime *t, MyTime *a )
{
	a->year        = t->year;
	a->month       = t->month ;
	a->jday        = t->jday ;
	...
	a->offset      = t->offset;
	a->dst         = t->dst;
	strcpy( a->tzone , t->tzone  );
	strcpy( a->cday  , t->cday   );
	strcpy( a->ampm  , t->ampm   );
	strcpy( a->cmonth, t->cmonth );
	return;
}
