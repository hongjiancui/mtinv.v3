...
/***************************************************************/
	...
	{
	  fprintf( stdout, 
		"%s: cross_correlation.c: xcorr(): ilag=%d tlag=%g xcorcoef=%g\n",
			progname,  
			lag, 
			xlag, 
			xcor );
...
/*******************************************/
	...
	if(verbose) 
	{
		fprintf( stdout, "%s: ishift=%d xcor=%g xlag=%g lag=%d cortol=%g\n",
			progname, ishift, xcor, xlag, lag, cortol );
	}
	...
	if( ishift && (xcor > cortol) )
	{
		if(verbose)
		{
		  fprintf( stdout, "%s: cross_correlation.c: xcorr(): shifting data lag=%d\n", 
			progname, lag );
		}
...
/*********************************/
	...
	if(verbose)printf("Autocorrelation x coefxx=%g\n", coefxx );
	free(c1);
...
/*********************************/
	...
	if(verbose)printf("Autocorrelation y coefyy=%g\n", coefyy );
	free(c1);
...
/*********************************************************/
	...
	if( coef1 >= coef2 )
	{
		...
		*cor = coef1/sqrt( coefxx * coefyy );
		if(verbose) 
		  printf("maxcor: coef1 >= coef2 cor=%g lag=%d\n",
			*cor, *lag );
		*lag *= -1;
	}
	...
	if( coef2 > coef1 )
	{
		...
		*cor = coef2/sqrt(coefxx*coefyy);
		if(verbose)
		  printf("maxcor: coef2 > coef1 cor=%g lag=%d\n",
		    *cor, *lag );
		*lag *= +1;
	}
	free(c1);
