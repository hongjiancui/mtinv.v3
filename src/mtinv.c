...
/****************************************************************/
	...
	/****************************/
		...
		if(verbose)
		{
		  fprintf( stdout, "%s: %s: %s: iz_best=%d z=%g ot=%g ts0=%g VR=%.2f pdc=%.0f fit1=%.2f fit2=%.2f\n",
			progname, 
			__FILE__, __func__,
                        iz_best, 
			z[iz_best], 
		...
		fprintf( stderr, "%s: %s: %s: iz_best=%d z=%g ot=%g ts0=%g VR=%.2f pdc=%.0f fit1=%.2f fit2=%.2f\n",
                        progname,
			__FILE__, __func__,
                        iz_best,
                        z[iz_best],
...
/******************************************************************/
	...
	if( igmt5 )
	{
		fprintf( stderr, "%s: mtinv(): calling plotmech_gmt5():\n", progname );
		if(verbose)
		{
...
/****************************************************************************************************/
	...
	{
	  fprintf( stdout,
		"%s: %s: %s: calling computeStationChannel_VarianceReduction_v2\n",
		progname, 
		__FILE__,
		__func__ );
	  fflush( stdout);
...
/******************************************************************/
	...
	normalize_moment_tensor( &Ma, &Mn, ev[ista].Mo, verbose );
...
/************************************************************************************************/
	...
	/*************************************************/
		...
		if( verbose ) 
		{
		  fprintf( stdout, "%s: mtinv.c:invert(): A matrix: rows=%d cols=%d\n", progname, rows, cols);
		}
	...
	/*************************/
		if(verbose)
		{
		  fprintf( stdout, "%s: mtinv.c:invert(): allocating memory for iz=%d\n", progname, iz );
		}

		if(verbose)
		{
		  fprintf( stdout, 
		    "%s: mtinv.c:invert(): Allocating memory for a_matrix rows=%d cols=%d\n", progname, rows, cols );
		}

		a_matrix  = matrix( 0, rows+1, 0, cols+1 );

		if(verbose)fprintf(stdout, "%s: mtinv.c:invert(): Allocating memory for u_matrix\n", progname );
		u_matrix  = matrix( 0, rows+1, 0, cols+1 );

		if(verbose)fprintf(stdout, "%s: mtinv.c:invert(): Allocating memory for v_matrix\n", progname );
		v_matrix  = matrix( 0, cols+1, 0, cols+1 );

		if(verbose)fprintf(stdout, "%s: mtinv.c:invert(): Allocating memory for cv_matrix\n", progname );
		cv_matrix = matrix( 0, cols+1, 0, cols+1 );

		if(verbose)fprintf(stdout, "%s: mtinv.c:invert(): Allocating memory for w_vector\n", progname );
		w_vector = vector( 0, cols+1 );

		if(verbose)fprintf(stdout, "%s: mtinv.c:invert(): Allocating memory for e_vector\n", progname );
		e_vector = vector( 0, cols+1 );

		if(verbose)fprintf(stdout, "%s: mtinv.c:invert(): Allocating memory for x_vector\n", progname );
		x_vector = vector( 0, cols+1 );

		if(verbose)fprintf(stdout, "%s: mtinv.c:invert(): Allocating memory for b_vector\n", progname );
		b_vector = vector( 0, rows+1 );

		if(verbose)fprintf(stdout, "%s: mtinv.c:invert(): Allocating memory for s_vector\n", progname );
		s_vector = vector( 0, rows+1 );
	...
	/******************************/
		...
		if( verbose )
		{
		  fprintf( stdout, "%s: mtinv.c:invert(): inverting... calling svdcmp() rows=%d cols=%d\n", 
			progname, rows, cols );
		}
	...
	/********************************************************************************************/
		...
		if( verbose )
		{
		  fprintf( stdout, "%s: mtinv.c:invert(): inverting... calling svbksb() rows=%d cols=%d\n", 
			progname, rows, cols );
		}
	...
	/****************************************************/
		...
		if(verbose) 
		{
			fprintf( stdout, "%s: mtinv.c:invert(): iz=%d %%VRED=%g\n", 
				progname, iz, sol[iz].var_red );
		}
	...
	/****************************************************/
		...
		if(verbose)
		{
		  fprintf(stdout, "%s: mtinv.c:invert(): iz=%d z=%g z=%g %%L2NORM=%g L2NORM/(NPTS-1)=%g NPTS=%d\n",
                     progname, iz, 
			sol[iz].evdp, 
			grn[0][iz].evdp,
			sol[iz].l2norm_error,
...
*/
		...
		if(verbose) fprintf( stdout, "%s: %s: %s: done loop iz=%d\n",
