...
unsigned int BestResult(char *path, char *pBest)
{
    ...
    printf("Max PVR: %1.4lf%%, File Name: %s, Dir: %s\n", maxpvr, maxpvr_filename, maxpvr_foldername);

    char best_result_path[1024];
    strcpy(best_result_path, path);
    strcat(best_result_path, "/best_result.txt");
    FILE *fpt;
    fpt = fopen(best_result_path, "w");
    fprintf(fpt, "Max PVR: %1.4lf%%, File Name: %s, Dir: %s\n", maxpvr, maxpvr_filename, maxpvr_foldername);
    fclose(fpt);
